...
import { users, systemSettings, registrationEvents, electiveGroups, electives, studentRegistrations } from "@/lib/db/schema";
import { eq, and, inArray, desc } from "drizzle-orm";
import { RegistrationForm } from "./registration-form";
import { ThemeToggle } from "@/components/theme-toggle";
import { logout } from "@/app/actions/auth";
import { LogOut, User, FileCheck, ShieldAlert, ArrowRight } from "lucide-react";
          ...
          <div className="w-full sm:w-auto bg-white dark:bg-[#111] rounded-md p-4 border border-slate-200 dark:border-white/10 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-medium text-slate-900 dark:text-white">
                {hasSubmitted ? "Choices Submitted" : "Pending Selection"}
              </p>
            </div>
            {hasSubmitted && (
              <Link href="/dashboard/success" className="ml-4 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1">
